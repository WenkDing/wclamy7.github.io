...
(function (global, factory) {
    ...
    histogram.thresholds = function (_) {
      return arguments.length
        ? ((threshold =
            typeof _ === "function"
              ? _
              : constant$b(Array.isArray(_) ? slice$3.call(_) : _)),
          histogram)
        : threshold;
    };
  ...
  function quantileIndex(values, p, valueof = number$3) {
    if (isNaN((p = +p))) return;
    numbers = Float64Array.from(values, (_, i) =>
      number$3(valueof(values[i], i, values))
    );
    ...
    var numbers,
      ...
      j = numbers.length - 1,
      i = Math.floor(j * p);
    quickselect(index, i, 0, j, (i, j) =>
      ascendingDefined(numbers[i], numbers[j])
    );
  ...
  let Path$1 = class Path {
    ...
    bezierCurveTo(x1, y1, x2, y2, x, y) {
      this._append`C${+x1},${+y1},${+x2},${+y2},${(this._x1 = +x)},${(this._y1 =
        +y)}`;
    }
    arcTo(x1, y1, x2, y2, r) {
      ...
      // Otherwise, draw an arc!
        ...
        this._append`A${r},${r},0,0,${+(y01 * x20 > x01 * y20)},${(this._x1 =
          x1 + t21 * x21)},${(this._y1 = y1 + t21 * y21)}`;
      }
    }
    arc(x, y, r, a0, a1, ccw) {
      (x = +x), (y = +y), (r = +r), (ccw = !!ccw);
      ...
      // Is this arc non-empty? Draw an arc!
        ...
        this._append`A${r},${r},0,${+(da >= pi$2)},${cw},${(this._x1 =
          x + r * Math.cos(a1))},${(this._y1 = y + r * Math.sin(a1))}`;
      }
    }
    rect(x, y, w, h) {
      this._append`M${(this._x0 = this._x1 = +x)},${(this._y0 = this._y1 =
  ...
  function index$2(projection, context) {
    ...
    path.projection = function (_) {
      ...
      projectionStream =
        _ == null ? ((projection = null), identity$5) : (projection = _).stream;
      return path;
    };
  ...
  function line(x$1, y$1) {
    ...
    x$1 =
      typeof x$1 === "function" ? x$1 : x$1 === undefined ? x : constant$1(x$1);
    y$1 =
  ...
  function link(curve) {
    ...
    function link() {
      ...
      output.lineStart();
      (argv[0] = s),
        output.point(+x$1.apply(this, argv), +y$1.apply(this, argv));
      (argv[0] = t),
        output.point(+x$1.apply(this, argv), +y$1.apply(this, argv));
      output.lineEnd();
      if (buffer) return (output = null), buffer + "" || null;
