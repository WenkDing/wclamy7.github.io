...
(function (global, factory) {
  ...
  AxisInternal.prototype.generateAxis = function () {
    ...
    axis.tickInterval = function () {
      ...
      if (params.isCategory) {
        ...
        length =
          axis.g.select("path.domain").node().getTotalLength() -
  ...
  Axis.prototype.getYAxis = function getYAxis(
    ...
    if ($$.isTimeSeriesY()) {
      axis.ticks(
        config.axis_y_tick_time_type,
        config.axis_y_tick_time_interval
      );
    } else {
      axis.tickValues(tickValues);
    }
  ...
  Axis.prototype.xForAxisLabel = function xForAxisLabel(
    forHorizontal,
    position
  ) {
    var $$ = this.owner;
  ...
  Axis.prototype.textAnchorForY2AxisLabel =
  ...
  Axis.prototype.updateLabels = function updateLabels(withTransition) {
    var $$ = this.owner;
    var axisXLabel = $$.main.select(
        "." + CLASS.axisX + " ." + CLASS.axisXLabel
      ),
      axisYLabel = $$.main.select("." + CLASS.axisY + " ." + CLASS.axisYLabel),
      axisY2Label = $$.main.select(
        "." + CLASS.axisY2 + " ." + CLASS.axisY2Label
      );
  ...
  Axis.prototype.convertPixelsToAxisPadding =
    function convertPixelsToAxisPadding(pixels, domainLength) {
      var $$ = this.owner,
  ...
  Axis.prototype.generateTransitions = function generateTransitions(duration) {
    ...
    return {
      ...
      axisSubX: duration
        ? axes.subx.transition().duration(duration)
        : axes.subx,
    };
  };
  ...
  ChartInternal.prototype.initParams = function () {
    ...
    $$.rotated_padding_right =
      config.axis_rotated && !config.axis_x_show ? 0 : 30;
  ...
  ChartInternal.prototype.updateSizes = function () {
    ...
    var legendHeight = $$.legend ? $$.getLegendHeight() : 0,
      ...
      subchartXAxisHeight =
        config.axis_rotated || hasArc
          ? 0
          : $$.getHorizontalAxisHeight("x", true),
      subchartHeight =
  ...
  ChartInternal.prototype.transformMain = function (
    withTransition,
    transitions
  ) {
    var $$ = this,
  ...
  ChartInternal.prototype.transformAll = function (
    withTransition,
    transitions
  ) {
    var $$ = this;
    ...
    if (!("SVGPathSeg" in window)) {
      ...
      if (!("getPathSegAtLength" in window.SVGPathElement.prototype)) {
        ...
        window.SVGPathElement.prototype.getPathSegAtLength = function (
          ...
          var lastPathSegment =
            measurementElement.pathSegList.numberOfItems - 1;
  ...
  ChartInternal.prototype.generateFlow = function (args) {
    ...
    return function () {
      ...
      wait.add(
        mainLine.transition(flowTransition).attr("transform", transform)
      );
      wait.add(
        mainArea.transition(flowTransition).attr("transform", transform)
      );
      wait.add(
        mainCircle.transition(flowTransition).attr("transform", transform)
      );
      wait.add(
        mainText.transition(flowTransition).attr("transform", transform)
      );
      wait.add(
        mainRegion
          .filter($$.isRegionOnX)
  ...
  ChartInternal.prototype.updateRadius = function () {
    ...
    $$.radiusExpanded =
      (Math.min($$.arcWidth, $$.arcHeight) / 2) *
      ($$.hasType("gauge") ? 0.85 : 1);
  ...
  ChartInternal.prototype.updateAngle = function (d) {
    ...
    if ($$.isGaugeType(d.data)) {
      ...
      gValue =
        d.value < gMin ? 0 : d.value < gMax ? d.value - gMin : gMax - gMin;
  ...
  ChartInternal.prototype.redrawArc = function (
    ...
    mainArc = arcs
      ...
      .style("cursor", function (d) {
        return config.interaction_enabled &&
          config.data_selection_isselectable(d)
          ? "pointer"
          : null;
      })
      .each(function (d) {
  ...
  ChartInternal.prototype.categoryName = function (i) {
    ...
    return i < config.axis_x_categories.length
      ? config.axis_x_categories[i]
      : i;
  };
  ...
  ChartInternal.prototype.classRegion = function (d, i) {
    return (
      this.generateClass(CLASS.region, i) +
      " " +
      ("class" in d ? d["class"] : "")
    );
  };
  ...
  ChartInternal.prototype.classFocused = function (d) {
    return (
      " " + (this.focusedTargetIds.indexOf(d.id) >= 0 ? CLASS.focused : "")
    );
  };
  ...
  ChartInternal.prototype.selectorTarget = function (id, prefix) {
    return (
      (prefix || "") + "." + CLASS.target + this.getTargetSelectorSuffix(id)
    );
  };
  ...
  ChartInternal.prototype.getClipPath = function (id) {
    return (
      "url(" + (isIE(9) ? "" : document.URL.split("#")[0]) + "#" + id + ")"
    );
  };
  ...
  ChartInternal.prototype.getYAxisClipX = function () {
    ...
    return $$.config.axis_y_inner
      ? -1
      : $$.getAxisClipX($$.config.axis_rotated);
  };
  ...
  ChartInternal.prototype.isStackNormalized = function () {
    return (
      this.config.data_stack_normalize && this.config.data_groups.length > 0
    );
  };
  /**
  ...
  ChartInternal.prototype.isTargetNormalized = function (targetId) {
    ...
    return (
      $$.isStackNormalized() &&
      $$.config.data_groups.some(function (group) {
  ...
  ChartInternal.prototype.dist = function (data, pos) {
    var $$ = this,
      ...
      x = $$.x(data.x);
    return Math.sqrt(
      Math.pow(x - pos[xIndex], 2) + Math.pow(y - pos[yIndex], 2)
    );
  };
  ...
  ChartInternal.prototype.load = function (targets, args) {
    ...
    if (targets) {
      ...
      if (args.type || args.types) {
        targets.forEach(function (t) {
          var type =
            args.types && args.types[t.id] ? args.types[t.id] : args.type;
      ...
      .on(
        ...
        config.interaction_enabled
          ? function () {
              ...
              var showSingleDataPoint =
                !config.tooltip_grouped ||
                $$.hasType("stanford", targetsToShow);
  ...
  ChartInternal.prototype.updateLegend = function (
    ...
    targetIds = targetIds.filter(function (id) {
      return (
        !isDefined(config.data_names[id]) || config.data_names[id] !== null
      );
    });
    options = options || {};
    withTransition = getOption(options, "withTransition", true);
    ...
    function updatePositions(textElement, id, index) {
      var reset = index === 0,
        ...
        itemWidth =
          box.width +
          tileWidth +
          (isLast && !($$.isLegendRight || $$.isLegendInset)
            ? 0
            : paddingRight) +
    ...
    rects = $$.legend
      .selectAll("rect." + CLASS.legendItemEvent)
      .data(targetIds);
  ...
  ChartInternal.prototype.getShapeX = function (
    ...
    return function (d) {
      ...
      return d.x || d.x === 0
        ? scale(d.x) - offset * (targetsNum / 2 - index)
        : 0;
    };
  };
  ...
  ChartInternal.prototype.getShapeOffset = function (
    typeFilter,
    indices,
    isSub
  ) {
  ...
  ChartInternal.prototype.updateTargetsForLine = function (targets) {
    ...
    targets.forEach(function (t) {
      $$.main
        .selectAll(
          "." + CLASS.selectedCircles + $$.getTargetSelectorSuffix(t.id)
        )
  ...
  ChartInternal.prototype.generateGetLinePoints = function (
    lineIndices,
    isSub
  ) {
  ...
  ChartInternal.prototype.generateDrawArea = function (areaIndices, isSub) {
    ...
    area = config.axis_rotated
      ? area.x0(value0).x1(value1).y(xValue)
      : area
          .x(xValue)
  ...
  ChartInternal.prototype.generateGetAreaPoints = function (
    areaIndices,
    isSub
  ) {
    // partial duplication of generateGetBarPoints
    ...
    return [
      (withTransition ? $$.mainCircle.transition(transition) : $$.mainCircle)
        ...
        .style(
          "color",
          $$.isStanfordGraphType()
            ? $$.getStanfordPointColor.bind($$)
            : $$.color
        )
        .attr("cx", cx)
        .attr("cy", cy),
      (withTransition
        ? selectedCircles.transition(transition)
        : selectedCircles
      )
        .attr("cx", cx)
        .attr("cy", cy),
  ...
  ChartInternal.prototype.getCircles = function (i, id) {
    ...
    return (
      id
        ? $$.main.selectAll(
            "." + CLASS.circles + $$.getTargetSelectorSuffix(id)
          )
        : $$.main
  ...
  ChartInternal.prototype.isWithinCircle = function (that, r) {
    var d3 = this.d3,
      ...
      cy = +d3_this.attr("cy");
    return (
      Math.sqrt(Math.pow(cx - mouse[0], 2) + Math.pow(cy - mouse[1], 2)) < r
    );
  };
  ...
  ChartInternal.prototype.getAxisWidthByAxisId = function (
    id,
    withoutRecompute
  ) {
    var $$ = this,
    ...
    return (
      $$.axis.getMaxTickWidth(id, withoutRecompute) +
      (position.isInner ? 20 : 40)
    );
  };
  ChartInternal.prototype.getHorizontalAxisHeight = function (
    axisId,
    isSubchart
  ) {
    var $$ = this,
    ...
    if (
      axisId === "x" &&
      !(isDefined(isSubchart) && isSubchart
    ...
    if (axisId === "x" && !config.axis_rotated && config.axis_x_tick_rotate) {
      h =
        ...
        $$.axis.getMaxTickWidth(axisId) *
          Math.cos(
            (Math.PI * (90 - Math.abs(config.axis_x_tick_rotate))) / 180
          );
    }
    ...
    if (axisId === "y" && config.axis_rotated && config.axis_y_tick_rotate) {
      h =
        ...
        $$.axis.getMaxTickWidth(axisId) *
          Math.cos(
            (Math.PI * (90 - Math.abs(config.axis_y_tick_rotate))) / 180
          );
    }
    return (
      h +
  ...
  ChartInternal.prototype.xForColorScale = function () {
    var $$ = this;
    return (
      $$.config.stanford_padding.right + getBBox($$.colorScale.node()).width
    );
  };
  ...
  ChartInternal.prototype.getStanfordTooltipTitle = function (d) {
    ...
    return (
      "\n      <tr><th>" +
      (labelX ? sanitise(labelX) : "x") +
      "</th><th class='value'>" +
      d.x +
      "</th></tr>\n      <tr><th>" +
      (labelY ? sanitise(labelY) : "y") +
      "</th><th class='value'>" +
      d.value +
      "</th></tr>\n    "
    );
  ...
  ChartInternal.prototype.getTooltipContent = function (
    ...
    for (i = 0; i < d.length; i++) {
      ...
      if ($$.isStanfordGraphType()) {
        ...
        if (!text) {
          title = sanitise(
            titleFormat ? titleFormat(d[i].x, d[i].index) : d[i].x
          );
          text =
            "<table class='" +
            $$.CLASS.tooltip +
        ...
        if (value !== undefined) {
          ...
          name = sanitise(
            nameFormat(d[i].name, d[i].ratio, d[i].id, d[i].index)
          );
  ...
  ChartInternal.prototype.tooltipPosition = function (
    ...
    if (forArc) {
      tooltipLeft =
        ($$.width - ($$.isLegendRight ? $$.getLegendWidth() : 0)) / 2 +
        mouse[0];
      tooltipTop =
  ...
  ChartInternal.prototype.showTooltip = function (selectedData, element) {
    ...
    position = positionFunction.call(
      this,
      dataToShow,
      tWidth,
      tHeight,
